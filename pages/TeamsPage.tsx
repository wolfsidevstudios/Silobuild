...
export const TeamsPage: React.FC = () => {
    ...
    return (
        <div className="p-8 h-full overflow-y-auto">
            ...
            {teams.length === 0 ? (
                <div className="flex flex-col items-center justify-center text-center text-gray-500 bg-white/5 rounded-lg p-8 mt-10">
                    <UsersIcon className="w-16 h-16 mb-4 text-gray-600"/>
                    <h2 className="text-xl font-semibold mb-2 text-white">No teams yet</h2>
                            ...
                            <div key={team.id} className="bg-white/5 border border-white/10 rounded-lg p-6">
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-400">Created on {new Date(team.createdAt).toLocaleDateString()}</p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => addMember(team.id)} className="text-gray-400 hover:text-white p-2 hover:bg-white/10 rounded-full"><PlusIcon /></button>
                                        <button onClick={() => deleteTeam(team.id)} className="text-gray-400 hover:text-red-400 p-2 hover:bg-white/10 rounded-full"><TrashIcon /></button>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <div className="space-y-2">
                                            {team.members.map(member => (
                                                <div key={member.sub} className="flex items-center gap-3 bg-black/20 p-2 rounded-md">
                                                    <img src={member.picture} alt={member.name} className="w-8 h-8 rounded-full" />
                                                    <div>
                                                        <p className="text-sm font-medium">{member.name}</p>
                                                        <p className="text-xs text-gray-400">{member.email}</p>
                                                    </div>
                                                    <span className="ml-auto text-xs bg-gray-700 text-gray-300 px-2 py-0.5 rounded-full capitalize">{member.role}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        {teamProjects.length > 0 ? (
                                            <div className="space-y-2">
                                                {teamProjects.map(p => (
                                                    <a key={p.id} href={`#/project/${p.id}`} className="flex items-center gap-3 bg-black/20 p-2 rounded-md hover:bg-black/40">
                                                         <div className="w-8 h-8 rounded-md bg-gray-700 flex-shrink-0 flex items-center justify-center border border-white/10">

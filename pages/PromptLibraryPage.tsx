...
const PromptCard: React.FC<PromptCardProps> = ({ prompt, onUse }) => {
  return (
    <div className="bg-white/5 border border-white/10 rounded-lg p-4 flex flex-col justify-between hover:border-blue-500/50 transition-colors duration-300">
      <div>
        <h3 className="font-bold text-white truncate">{prompt.title}</h3>
        <p className="text-sm text-gray-400 mt-1 h-10 line-clamp-2">{prompt.description}</p>
      </div>
      <button
        onClick={() => onUse(prompt.prompt)}
        className="mt-4 w-full bg-white/10 text-white px-3 py-1.5 rounded-full text-xs font-semibold hover:bg-blue-600 hover:text-white transition-colors"
      >
        Use this prompt
      </button>
    </div>
    ...
    <div className="p-8 h-full overflow-y-auto">
      ...
      <div className="space-y-10">
        {categoryOrder.map(category => (
          <div key={category}>
            <h2 className="text-xl font-semibold mb-4 border-b border-white/10 pb-2">{category}</h2>
